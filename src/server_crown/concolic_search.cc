...
int Search::LaunchProgram(const vector<Value_t>& inputs) {
	...
	ret = exec_cil_in_rasp(); 
    return ret;
}

void Search::send_Msg(char* message){

	char cmd[MAX];

	memset(cmd, 0x00, MAX);
	sprintf(cmd, "%s%d%s%s", "tcp_server_send_msg ",rasp_port," ", file_name);
	system(cmd);
}

void Search::send_File(char* file_name){

	char cmd[MAX];
	
	memset(cmd, 0x00, MAX);
	sprintf(cmd, "%s%d%s%s", "tcp_server_send_file ",rasp_port," ", file_name);
	system(cmd);

}

int Search::exec_cil_in_rasp(){

	char cmd[MAX];
	memset(cmd, 0x00, MAX);
	sprintf(cmd, "%s%d", "tcp_server ",rasp_port);
	system(cmd);
