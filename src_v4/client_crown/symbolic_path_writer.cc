...
void SymbolicPathWriter::Serialize(ostream &os) const{
	...
	//send_server_path( "path", strlen("path"));
	//memset(num_path, 0x00, size_MAX);
	//sprintf(num_path,"%d",branches_.front());
	//printf(">num_path %d %d\n", branches_.front(), branches_.size() * sizeof(branch_id_t));
	//os.write((char*)&branches_.front(), branches_.size() * sizeof(branch_id_t)); send_server_path( nbr, strlen(nbr));send_server_path((char*)&branches_.front(), branches_.size() * sizeof(branch_id_t));//num_path, strlen(num_path));

	for(int i = 0; i<branches_.size(); i++){
		//printf("path %d\n",hyorm++);
		send_server_path( "path", strlen("path"));
		memset(num_path, 0x00, size_MAX);
		sprintf(num_path,"%d",branches_.at(i));
		os.write((char*)&branches_.at(i), sizeof(branch_id_t));
		send_server_path( "int", strlen("int"));
		send_server_path(num_path, strlen(num_path));
		//send_server_path((char*)&branches_.at(i), sizeof(branch_id_t));
	}
	...
	//send_server_path( "path", strlen("path"));
	//memset(num_path, 0x00, size_MAX);
	//sprintf(num_path,"%d",constraints_idx_.front());
	//printf("path %d\n",hyorm++);
	//printf(">num_path %d %d\n", constraints_idx_.front(),constraints_.size() * sizeof(size_t));
	//os.write((char*)&constraints_idx_.front(), constraints_.size() * sizeof(size_t)); send_server_path( nbr, strlen(nbr));send_server_path((char*)&constraints_idx_.front(), constraints_.size() * sizeof(size_t));//num_path, strlen(num_path));
   	
       for(int i = 0; i<constraints_.size();i++){
       		//printf("path %d\n",hyorm++);
       		send_server_path( "path", strlen("path"));
		memset(num_path, 0x00, size_MAX);
		sprintf(num_path,"%d",constraints_idx_.at(i));
		os.write((char*)&constraints_idx_.at(i), sizeof(size_t));
		send_server_path("size_t",strlen("size_t"));
		send_server_path(num_path, strlen(num_path));
		//send_server_path((char*)&constraints_idx_.at(i), sizeof(size_t));
       }	
