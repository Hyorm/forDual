...
void BinExprWriter::Serialize(ostream &os) const {
	...
	os.write((char *)&binary_op_, sizeof(char));send_server_bin("char", strlen("char"));send_server_bin((char *)&binary_op_, sizeof(char)+1);
	left_->Serialize(os);
	right_->Serialize(os);
}

void BinExprWriter::send_server_bin(char* message, int cap) const{

	//printf("send_server_Bin message:\n");
	addrinfo hints, *p;
