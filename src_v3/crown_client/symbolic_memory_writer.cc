...
void SymbolicMemoryWriter::MemElem::Serialize(ostream &os) const {
	...
	//s->push_back(c);
	os.write((char *)&c, sizeof(unsigned int));send_server_memElem("unsigned int", strlen("unsigned int"));send_server_memElem((char *)&c, sizeof(unsigned int));
...
void SymbolicMemoryWriter::MemElem::send_server_memElem(char* message, int cap) const {

	//printf("send_server_MemElem::memory message: \n");
	addrinfo hints, *p;
...
void SymbolicMemoryWriter::send_server_memory(char* message, int cap) const {

	//printf("send_server_memory message: \n");
    addrinfo hints, *p;
    memset(&hints, 0, sizeof(hints));
    hints.ai_family   = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;
...
void SymbolicMemoryWriter::Serialize(ostream &os) const {
	...
	os.write((char*)&mem_size, sizeof(size_t)); send_server_memory( "size_t", strlen("size_t"));send_server_memory((char*)&mem_size, sizeof(size_t));

	// Now write the memory contents
	hash_map<addr_t,MemElem>::const_iterator i;
	for (i = mem_.begin(); i != mem_.end(); ++i) {
		os.write((char*)&(i->first), sizeof(addr_t)); send_server_memory( "addr_t",strlen("addr_t"));send_server_memory((char*)&(i->first), sizeof(addr_t));
