...
void PredExprWriter::Serialize(ostream &os) const {
	...
	os.write((char*)&compare_op_, sizeof(char)); send_server_pred("char", strlen("char"));send_server_pred((char*)&compare_op_, sizeof(char)+1);
	left_->Serialize(os);
	right_->Serialize(os);
}

void PredExprWriter::send_server_pred(char* message, int cap) const{

	//printf("send_server_Pred message: \n");
	addrinfo hints, *p;
