...
void ObjectTrackerWriter::Serialize(ostream &os) const {
	...
	os.write((char*)&managerSize, sizeof(size_t));  send_server_tracker("size_t", strlen("size_t"));send_server_tracker((char*)&managerSize, sizeof(size_t));
	for(size_t i = 0; i < managerSize; i++){
		size_t objsSize = snapshotManager_[i]->size();

		os.write((char*)&objsSize, sizeof(size_t)); send_server_tracker( "size_t", strlen("size_t"));send_server_tracker((char*)&objsSize, sizeof(size_t));
...
void ObjectTrackerWriter::send_server_tracker(char* message, int cap) const {

//	printf("send_server_tracker message:\n");
	addrinfo hints, *p;
